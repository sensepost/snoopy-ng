...
class Snoopy():
	...
	def __init__(self,moduleNames,dbms="sqlite:///snoopy.db",server="http://localhost:9001/",drone="unnamedDrone",key=None,location="unknownLocation"):
		...
		self.all_data={}
		self.RUN=True
		self.SYNC_FREQ=5	#Sync every 5 seconds
		self.server=server
		self.drone=drone
		self.location=location
		self.key=key
		self.run_id=''.join(random.choice(string.ascii_uppercase + string.digits) for x in range(10))
		#Database
		self.tables={}
	...
	@staticmethod
	def get_plugins():
		moduleNames=[ os.path.basename(f)[:-3] for f in glob.glob("./plugins/*.py") if not os.path.basename(f).startswith('__')]
		return moduleNames
		...
		for m in self.modules:
			...
			for rawdata in multidata:
				if rawdata != None and len(rawdata) >0:
					tbl=rawdata[0]
					data=rawdata[1]
					if tbl not in self.all_data:
						self.all_data[tbl]=data
					else:
						self.all_data[tbl]+=data
...
#				print "Found %d rows to sync in table %s" %(len(results),table)
                                if len(results) > 0:
                                        ...
                                        data_to_upload.append( {"table":table_name, "data":result_as_dict} )
                        else:  
                ...
                if len(data_to_upload) > 0:
                        ...
                        if sync_success:
                                logging.debug("Successfully sync'd %d rows of data :)"%total_rows_to_upload)
                                for table_name in self.tables:#self.metadata.sorted_tables:
                                        table=self.tables[table_name]
                                        if "sunc" in table.c:
                                ...
                                logging.debug("Error attempting to upload %d rows of data :("%total_rows_to_upload)


        def web_upload(self,json_data):
		
		headers = {'content-type': 'application/json', 'Z-Auth':self.key, 'Z-Drone':self.drone}
...
#	parser.add_option("-p", "--parameters", dest="parameters", action="append", help="Optional module parameters, per module. Omit for default.")
	...
	if (options.newdrone ):
		...
		key=includes.webserver.webserver.manage_drone_account(options.newdrone,"create",options.dbms)
		print "[+] Key for '%s' is '%s'"%(options.newdrone,key)
		print "[+] Use this value in client mode to sync data to a remote server. e.g:"
		print "    %s --client http://remote-server/ --drone %s --key %s --location <somelocation> -m <module01> -m <modules02>"%(__file__,options.newdrone,key)
		sys.exit(0)
	elif ( options.deldrone ):
		import includes.webserver
		print "[+] Deleting drone account '%s'"%options.deldrone
		includes.webserver.webserver.manage_drone_account(options.deldrone,"delete",options.dbms)
		sys.exit(0)

	elif( options.listdrones ):
		import includes.webserver
		print "[+] Available drone accounts:"
		drones=includes.webserver.webserver.manage_drone_account(1,"list",options.dbms)
		for d in drones:
			print "\t%s:%s" %(d[0],d[1])
		sys.exit(0)

	if( (options.sync_server != None and options.server_port != None) or (options.sync_server == None and options.server_port == None) ):
	...
	#Client mode
	snoop_dog=None		
	if( options.sync_server != None ):

		if options.drone == None or options.location == None :
		...
		for m in mods:
			if m.split(":",1)[0] not in moduleNames:
				print "Error: Invalid module - '%s'. Use --list to list all available modules."%(m.split(":",1)[0])
				sys.exit(-1)
		print "[+] Starting Snoopy with modules: %s"%(str(mods))
		snoop_dog=Snoopy(mods,options.dbms,options.sync_server,options.drone,options.key,options.location)

	#Server mode
	else:
		print "[+] Starting Snoopy sync web server. Listening on port '%d' with sync web path '%s'"%(options.server_port,options.server_path)
		import includes.webserver
                snoop_dog=includes.webserver.webserver(options.dbms,options.server_path,options.server_port)
