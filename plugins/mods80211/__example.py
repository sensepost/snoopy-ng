...
class Snarf():
    ...
    def get_tables():
        ...
        table = Table('mac_guids', MetaData(),
    ...
    def proc_packet(self, p):
        if pkt.haslayer(TCP) and hasattr(pkt[TCP], 'load'):
            data=pkt[TCP].load
            mac = re.sub(':', '', p.addr2)
            ...
            if srl:
                guid = srl.group(1)
                if (mac, ssid) not in self.mac_guids:
                    self.mac_guids[(mac, guid)] = 0

    def get_data(self):
        tmp = []
        todel = []
        for k, v in self.mac_guids.iteritems():
            if v == 0:
                tmp.append( {"mac": k[0], "guid": k[1]} )
                todel.append((k[0], k[1]))

        # Reduce mac:ssid data structure if it's getting too large
        if len(self.mac_guids) > MAX_NUM_GUIDs:
            logging.debug("MAC:GUID structure is large (%d), going to reduce by 50pc (-%d)" % (len(self.mac_guids),(int(0.5*MAX_NUM_GUIDs))))
            for i in range(int(0.5 * MAX_NUM_GUIDs)):
                try:
                    self.mac_guids.popitem(last = False)
                except KeyError:
                    pass
            logging.debug("MAC:GUID structure is now %d" % len(self.mac_guids))

        if len(todel) > 0:
            for foo in todel:
                mac, ssid = foo[0], foo[1]
                self.mac_guids[(mac, ssid)] = 1
            return ("mac_guids", tmp)
